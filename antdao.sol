...
contract ANTDAOFaucet is TimeHelpers, Ownable {
    ...
    bytes32 public ANTDAOContext;
    address public ANTDAOVerifier;
    ...
    event Initialize(
        ...
        bytes32 ANTDAOContext,
        address ANTDAOVerifier,
        uint256 minimumEthBalance,
        address uniswapExchange
    );
    event SetPercentPerPeriod(uint256 percentPerPeriod);
    event SetANTDAOSettings(bytes32 ANTDAOContext, address ANTDAOVerifier);
    ...
    * @param _ANTDAOContext ANTDAO context used for verifying users
    * @param _ANTDAOVerifier ANTDAO verifier address that signs ANTDAO verifications
    ...
    */
    constructor(
        ...
        bytes32 _ANTDAOContext,
        address _ANTDAOVerifier,
        ...
        public
    {
        ...
        ANTDAOContext = _ANTDAOContext;
        ANTDAOVerifier = _ANTDAOVerifier;
    ...
    */
    ...
    /**
    * @notice Set ANTDAO settings
    * @param _ANTDAOContext ANTDAO context used for verifying users
    * @param _ANTDAOVerifier ANTDAO verifier address that signs ANTDAO verifications
    */
    function setANTDAOSettings(bytes32 _ANTDAOContext, address _ANTDAOVerifier) public onlyOwner {
        ANTDAOContext = _ANTDAOContext;
        ANTDAOVerifier = _ANTDAOVerifier;
        emit SetANTDAOSettings(_ANTDAOContext, _ANTDAOVerifier);
    }
    ...
    * @param _ANTDAOContext The context used in the users verification
    * @param _addrs The history of addresses, or contextIds, used by this user to register with ANTDAO for the ANTDAO context
    * @param _timestamp The time the verification was created by a ANTDAO node
    * @param _v Part of the ANTDAO nodes signature verifying the users uniqueness
    * @param _r Part of the ANTDAO nodes signature verifying the users uniqueness
    * @param _s Part of the ANTDAO nodes signature verifying the users uniqueness
    */
        ...
        public
    {
        ...
        require(_isVerifiedUnique(_ANTDAOContext, _addrs, _timestamp, _v, _r, _s), ERROR_INCORRECT_VERIFICATION);
    ...
    *         verified or the ANTDAO node providing verifications is down.
    */
    ...
    */
    ...
    {
        bytes32 signedMessage = keccak256(abi.encodePacked(_ANTDAOContext, _addrs, _timestamp));
        address verifierAddress = ecrecover(signedMessage, _v, _r, _s);

        bool correctVerifier = ANTDAOVerifier == verifierAddress;
        bool correctContext = ANTDAOContext == _ANTDAOContext;
    ...
    function _voidUserHistory(address[] memory _addrs) internal {
        ...
        // Void all previously used addresses to prevent users from registering multiple times using old ANTDAO verifications.
        uint256 index = 1;
